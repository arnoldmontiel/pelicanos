...
<div class="buscarAsociacion">
    ...
    <div class="col-sm-9">	
                            <input id="fieldSearchName" type="text" class="form-control" placeholder="<?php echo (!empty($query))?$query:'Título de la serie';?>">
                            </div>
                  </div>
...
<div id="list-movies" class="list-group scrollable-list">
		...
		{
			$date = date_parse($serie->release_date);
			$date = " (".$date['year'].")";
			echo "<a id='".$serie->id."' href='#' class='list-group-item'>".$serie->original_title.$date."</a>";
		}
		?>  
</div>
      </div>
  ...
  bindActions();
	$('#btn_search').click(function(){
		...
		$.post("<?php echo NzbController::createUrl('AjaxSearchSerieTMDB'); ?>",
