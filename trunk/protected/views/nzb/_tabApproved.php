<div class="searchOverTab"><button onclick="searchTabApproved('');" type="button" class="resetSearchBtn" ><i class="fa fa-times-circle"></i></button><input onchange="searchTabApproved(this.value);" type="text" value="<?php echo $filter;?>" class="form-control" placeholder=" Buscar Pel&iacute;cula"></div>
	<?php 
		foreach($modelNzbApproved as $item)
		{
			...
			if(isset($modelAutoRipper))
			{
				...
				if(isset($item->Id_my_movie_disc_nzb))
				{
					...
					echo '<td valign="top" class="tdImage"><a onclick="viewVideoInfo('.$modelAutoRipper->Id.');" data-toggle="modal" ><img class="movieImage" src="'.$poster.'" width="100"/><i class="fa fa-info-circle iconOverlay"></i></a></td>';
					echo '<td>';
